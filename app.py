...
# Load Data & Context for the sidebar
...
# Main Navigation - Multi-tab layout
...
with tab2:
    ...
    with st.form("add_plant_form", clear_on_submit=True):
        ...
        submitted = st.form_submit_with_button("Save Plant to Collection")
...
with tab3:
    ...
    if plants_df.empty:
        ...
        # Create dictionary map for selectbox display name -> id
        ...
        # Contextual input for Stretch Goal 1 (Growth tracking metric)
        measurement = None
...
with tab4:
    ...
    # Search Engine UI Component
    search_term = st.text_input("🔍 Filter assets dynamically by Name or Location", "")
    
    if search_term:
        display_df = pf.search_plants(search_term)
    else:
        display_df = plants_df.copy() # by defualt it show all, unless there's search term
        
    if display_df.empty:
        st.write("No matching plant inventories located.")
    else:
        # Display list layout with options to view specific history
        for _, plant in display_df.iterrows():
            ...
            with st.expander(f"🪴 {plant['name']} — Location: {plant['location']}"):
                ...
                with col1:
                    ...
                    st.write(f"**Baseline Watering Interval:** Every {plant['watering_frequency_days']} days")
                    st.write(f"**Lighting Preference:** {plant['sunlight_needs']}")
                    st.write(f"**Last Recorded Height:** {plant['current_height_cm']} cm")
                    if plant['photo_path'] and plant['photo_path'] != "None":
                        st.caption(f"📁 Linked Image Path: `{plant['photo_path']}`")
                
                with col2:
                    st.markdown("**Historical Interventions**")
                    plant_history = log_df[log_df["plant_id"] == str(pid)]
                    if plant_history.empty:
                        st.caption("No logging data saved for this plant yet.")
                    else:
                        st.dataframe(
                        ...
                        # Render simple linear growth trend if records exist
                        growth_history = plant_history[plant_history["activity_type"] == "Growth Tracking"].dropna(subset=["measurement_cm"])
...
# TAB 5: Plant Doctor 
with tab5:
    st.header("🩺 Symptom Diagnostics Engine")
    st.markdown("Select any visual anomalies your houseplant is currently displaying to check for probable care issues.")
    
    selected_symptoms = st.multiselect(
        "What specific problems are you observing?",
        ["Yellow leaves", "Mushy stems", "Crispy brown tips", "Drooping leaves", "Long, stretchy/skinny stems", "Webbing or tiny spots on leaves"]
    )
    
    if st.button("Run Diagnostic Scan"):
        if not selected_symptoms:
            st.info("Please pick one or more symptoms from the dropdown selector above to diagnose.")
        else:
            results = pf.diagnose_symptoms(selected_symptoms)
            st.markdown("### 📋 Evaluation Findings:")
